...
!!!============TITLE: BIG DATA VISUALIZATION OF LARGE AND ULTRA LARGE SCALE ATOMISTIC SIMULATIONS (UPTO 20 MILLION ATOMS) =================!!!
...
IMPLICIT NONE
	...
	real, dimension(:,:), allocatable::iprop_arr, temp_arr, temp_vm_arr
	real,dimension(:), allocatable::atomid_arr, vm_s,vm_vol_arr,vm_struct_arr
	real, dimension(:,:,:), allocatable::avg_props
	real::vm_v,x,y,z,xy,yz,zx,Macro_VM
	character(120)::Output_filename
	!!================================!!
	...
	read(201,*)
	read(201,*) Output_filename		!!Specify the Output File Name
	close(201)
...
25 continue
	...
	allocate(temp_vm_arr(6,structmax+4))
	allocate(vm_vol_arr(structmax+4))
	allocate(vm_struct_arr(structmax+4))
	p = 1
	r = 1
	do r = 1,structmax+4	
		temp_vm_arr(:,r)=0.0	
		vm_vol_arr(r)=0.0	
		do p = 1,grnmax
			!!Calculating the Average Von Mises Stress w.r.t structure type
			temp_vm_arr(:,r) = temp_vm_arr(:,r) + avg_props(r,4:9,p)	!FOR VM_STRUCT_ARR
			vm_vol_arr(r) = vm_vol_arr(r) + avg_props(r,10,p)	!FOR VM_STRUCT_ARR
			!For Von Mises over structure type
		...
		!!CALCULATING THE VON MISES STRESS SEPARATELY FOR EACH STRUCTURE TYPE:VM_STRUCT_ARR
		temp_vm_arr(:,r) = temp_vm_arr(:,r)/vm_vol_arr(r)
		vm_struct_arr(r) = SQRT((0.5)*((temp_vm_arr(1,r)-temp_vm_arr(2,r))**2 + (temp_vm_arr(2,r)-temp_vm_arr(3,r))**2 + &
		& (temp_vm_arr(3,r)-temp_vm_arr(1,r))**2)+ 3 *(temp_vm_arr(4,r)**2+temp_vm_arr(5,r)**2+temp_vm_arr(6,r)**2))	
		!VM_STRUCT_ARR CONTAINS THE VON MISES STRESSES FOR EACH STRUCTURE TYPE INDIVIDUALLY.		
	end do
	!!!=============================================================================!!!
	!!!=============================================================================!!!
	...
	open(210, file = Output_filename, status='new')  !!!***THE FILE NAME INTO WHICH THE DATA IS TO BE WRITTEN IS SPECIFIED***!!!
	...
	& in blocks of structure types: 0=Other Unknown Defects,1=FCC,2=HCP,3=BCC,...&
	...
	do u = 1,structmax+4
		do o = 1,grnmax
			if (o==1)then
				...
				write(210,*)"#AVERAGE VON MISES STRESS FOR THIS STRUCTURE TYPE IS",vm_struct_arr(u)
			end if
			write(210,*)o,avg_props(u,:,o)
		end do
	end do
	close(210)
	print*, "DEAR USER!!! CHECK OUT THE AVERAGE PROPERTIES BY OPENING::<",trim(Output_filename),">FILE!!!"
	
	!!!**THE ANALYSIS IS COMPLETE*THE RESULTS ARE STORED IN A FILE FOR FURTHER VISUAL ANALYSIS**!!!
	!***END PROGRAM***!
end program Big_Data_Avg
