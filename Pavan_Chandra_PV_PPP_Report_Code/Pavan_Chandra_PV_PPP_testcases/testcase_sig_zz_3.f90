	...
	do t = 1,grnmax
		do u=1,size(atomid_arr)
			if (iprop_arr(6,u)==t) then
				iprop_arr(7:12,u)=(/ 0.0, 0.0, 3.0*t, 0.0, 0.0, 0.0/)	!!**sigma_xx=1.0*grain num, and other stresses are 0**!!
				iprop_arr(13,u)=0.5	!!**the volume is 0.5 and remains constain for each grain**!!
			end if
		end do
	end do	
	!!** OPEN THE FILE FOR WRITING THE DATA**!!
	open(204, file = 'DATA_FOR_TESTCASE_3flt.txt', status='new') 
	write(204,*)"#Personal Programming Project---Pavan Chandra Vundurthy(62750)---under the supervision of Dr. Arun Prakash"
	write(204,*)"#Below is the Generated data for the test case_3 with corresponding stresses to check for the analysis"
	write(204,*)"=============================================================================================="""
	o = 1
